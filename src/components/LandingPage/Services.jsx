  ...
  useEffect(() => {
    const handleScroll = () => {
      sectionRefs.current.forEach((section) => {
        ...
        if (image && scrollProgress >= 0 && scrollProgress <= 1) {
  ...
  return (
    <section id="services" className="services-vertical">
      ...
      <div className="progress-indicator">
        ...
        <div className="progress-labels">
          {services.map((service, index) => (
            <button
              key={service.id}
              className={`progress-label ${index === activeSection ? "active" : ""}`}
              onClick={() => {
                sectionRefs.current[index]?.scrollIntoView({ behavior: "smooth", block: "center" })
              }}
              style={{
                color: index === activeSection ? service.color : "#999",
              }}
            >
              <span className="label-number">{String(index + 1).padStart(2, "0")}</span>
      ...
      <style>{`
        ...
        .service-section:hover .service-image-container {
